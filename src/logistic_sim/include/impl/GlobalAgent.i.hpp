...
bool GlobalAgent::token_check_pt(std::vector<uint> &my_path, std::vector<logistic_sim::Path> &other_paths, uint CHECK_ID, int *id_vertex_stuck)
{
	bool status = true;
...
std::vector<unsigned int> GlobalAgent::spacetime_dijkstra(const std::vector<std::vector<unsigned int> > &other_paths, const graph_type &graph, unsigned int size, const std::vector<unsigned int> &waypoints)
{
	std::cout << "WAYPOINTS:";
...
std::vector<uint> GlobalAgent::token_dijkstra(const std::vector<uint> &waypoints, std::vector<logistic_sim::Path> &other_paths)
{
