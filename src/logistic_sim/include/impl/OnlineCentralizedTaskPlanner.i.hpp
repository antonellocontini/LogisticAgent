...
template <class T>
unsigned int OnlineCentralizedTaskPlanner::insertion_sort(st_location *queue, unsigned int size, st_location loc, T *cmp_function)
{
  int i;
  for (i = size - 1; i >= 0; i--)
  {
...
template <class T>
std::vector<unsigned int> OnlineCentralizedTaskPlanner::spacetime_dijkstra(const std::vector<logistic_sim::Path> &other_paths,
