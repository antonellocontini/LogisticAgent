...
#include "OnlineRecoveryMATaskPlanner.hpp"
...
{
OnlineRecoveryMATaskPlanner::OnlineRecoveryMATaskPlanner(ros::NodeHandle &nh_, const std::string &name)
...
void OnlineRecoveryMATaskPlanner::token_callback(const logistic_sim::TokenConstPtr &msg)
{
...
void OnlineRecoveryMATaskPlanner::init_token(const logistic_sim::TokenConstPtr &msg, logistic_sim::Token &token)
{
  token.HEADER.seq = 1;
...
void OnlineRecoveryMATaskPlanner::multi_agent_repair(const logistic_sim::TokenConstPtr &msg, logistic_sim::Token &token)
{
  // get valid paths
  ...
  // std::vector<uint> feasible_vertices, unfeasible_vertices;
  // for (uint i=0; i<map_graph.size(); i++)
  // {
  //   if (check_feasible_recovery_vertex(i) && i != src_vertex && std::find(dst_vertex.begin(), dst_vertex.end(), i) == dst_vertex.end())
  //   {
  //     feasible_vertices.push_back(i);
  //   }
  //   else
  //   {
  //     unfeasible_vertices.push_back(i);
  //   }
  // }
  // std::vector<uint> destination = recovery_search_tree.find_recovery_configuration(other_paths, robot_ids, bind(&OnlineRecoveryMATaskPlanner::check_valid_recovery_configuration, this, _1, _2, _3, unfeasible_vertices));
...
void OnlineRecoveryMATaskPlanner::advertise_change_edge_service(ros::NodeHandle &nh)
{
  ROS_DEBUG_STREAM("Advertising change_edge service");
  change_edge_service = nh.advertiseService("change_edge", &OnlineRecoveryMATaskPlanner::change_edge, this);
...
void OnlineRecoveryMATaskPlanner::print_graph()
{
...
std::vector<std::vector<unsigned int>> OnlineRecoveryMATaskPlanner::build_graph()
{
...
bool OnlineRecoveryMATaskPlanner::change_edge(logistic_sim::ChangeEdge::Request &msg, logistic_sim::ChangeEdge::Response &res)
{
  int result;
...
std::vector<bool> OnlineRecoveryMATaskPlanner::_check_conflict_free_impl(uint task_endpoint)
{
  int found_count = 0;
...
bool OnlineRecoveryMATaskPlanner::check_conflict_free_property()
{
...
bool OnlineRecoveryMATaskPlanner::check_feasible_recovery_vertex(uint vertex)
{
  return reachability_test(src_vertex, dst_vertex, {vertex}, vertex_web);
}


bool OnlineRecoveryMATaskPlanner::check_valid_recovery_configuration(const std::vector<uint> &configuration,
                            ...
                            const std::vector<uint> &unfeasible_vertices)
{
  ...
  {
    if (std::find(unfeasible_vertices.begin(), unfeasible_vertices.end(), configuration[i]) != unfeasible_vertices.end())
    {
      return false;
    }
  }
...
void enumerate_configs(std::vector<std::vector<uint> > &result, std::vector<uint> &temp_config, const std::vector<uint> &vertices, uint robot_number, uint iteration = 0)
{
  for (uint i : vertices)
  {
    ...
    {
      enumerate_configs(result, temp_config, vertices, robot_number, iteration + 1);
    }
    else
    {
...
std::vector<std::vector<uint> > OnlineRecoveryMATaskPlanner::find_all_recovery_configs(const std::vector<std::vector<uint> > &waypoints, const std::vector<uint> &robot_ids)
{
  ...
  enumerate_configs(configurations, temp_config, feasible_vertices, robot_number);

  for (const std::vector<uint> &conf : configurations)
  {
    if (check_valid_recovery_configuration(conf, robot_ids, waypoints, unfeasible_vertices))
    {
...
std::vector<uint> OnlineRecoveryMATaskPlanner::find_best_recovery_config(const std::vector<std::vector<uint> > &waypoints
                                                                 , const std::vector<uint> &robot_ids
...
void OnlineRecoveryMATaskPlanner::build_fw_matrix()
{
...
int main(int argc, char *argv[])
{
  ...
  ros::NodeHandle nh_;
  onlinerecoverymataskplanner::OnlineRecoveryMATaskPlanner ODTP(nh_);
  ODTP.init(argc, argv);
