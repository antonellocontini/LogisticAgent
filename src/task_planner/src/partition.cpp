...
partition::iterator::iterator(unsigned n, bool first) : kappa(n), M(n)
{
  if (n <= 0)
...
partition::iterator &partition::iterator::operator++()
{
...
void partition::iterator::integrityCheck()
{
  const unsigned n = size();
...
// std::ostream &operator<<(std::ostream &out, partition::iterator &it)
// {
//   out << '(';

//   if (it->size() > 1)
//     std::copy(it->begin(), it->end() - 1, std::ostream_iterator<char>(out, " "));
...
partition::iterator_k::iterator_k(unsigned n, unsigned psize, bool first) : iterator(n, first), psize(psize)
{
  if (psize <= 0 || psize > n)
...
partition::iterator_k &partition::iterator_k::operator++()
{
  const unsigned n = size();
...
void partition::iterator_k::integrityCheck()
{
